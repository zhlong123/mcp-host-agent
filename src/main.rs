...
use clap::Parser;
use config::{CliArgs, RuntimeConfig, validate_config};
use paths::{mcp_err, resolve_allowed_path};
use rmcp::{
    ServerHandler,
...
struct WriteFileArgs {
    path: String,
    content_b64: String,
    /// Accepted for protocol compatibility; agent mode does not enforce mtime conflicts.
        ...
        if let Ok(meta) = tokio::fs::metadata(&path).await {
            if meta.len() as usize > max {
                return Err(mcp_err(
                    -32007,
                    format!(
                        "file too large: {} bytes (max_read_bytes={max})",
                        meta.len()
                    ),
                ));
            }
...
async fn health(State(state): State<Arc<AppState>>) -> Json<HealthResponse> {
    Json(HealthResponse {
        ...
        roots: state
            ...
            .collect(),
        mcp: "/mcp",
        ping: "MCP tool ping or GET /health for tunnel probes",
    })
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let runtime = config::load_config(&cli)?;
    ...
    let canon_roots = paths::canonical_roots(&runtime.roots);
    if !runtime.roots.is_empty() {
        info!(
            "path sandbox: {} root(s) configured",
            canon_roots.len()
        );
